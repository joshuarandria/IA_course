...
def value_of(mdp, s, a, v, gamma):
    ...
    for s2 in mdp.successor_states(s, a):
        sum += mdp.P(s, a, s2)*(mdp.R(s, a, s2)+gamma*v[s])
    return sum
...
def value_iteration(mdp, gamma, epsilon=0.001):
    ...
    n = 0
    v = {(0,0)}
    def steptwo(mdp,gamma,n):
      for s in mdp.states():
        vnp = 0
        for a in mdp.applicable_actions(s):
            if (value_of(mdp, s, a, v, gamma) >= vnp):
                vnp = value_of(mdp, s, a, v, gamma)
        returnValue[s]=vnp
      return returnValue
    goToStepFour=False
    while (goToStepFour==False):
           vnp=steptwo(mdp,gamma,n)
           print("dict vnp")
           print(vnp)
           for s in mdp.states():
                  if (abs(vnp[s]-v[s]) >= (epsilon*(1-gamma))/2*gamma):
                         n+=1
                         v=vnp
                         break
