...
def astar(start_state, goaltest, h):
    ...
    # g,Q,predecessor, start_state, goalstate,h
    ...
    g[start_state] = 0
    # print("g = "+str(g[start_state]))
    # print("h = "+str(h(start_state)))
    # print("f = "+str(h(start_state)+g[start_state]))
    # Insert the initial state in the queue
    Q.put((h(start_state)+g[start_state], start_state))

    g[goaltest] = inf
    while not Q.empty():
        path, state = Q.get()  # Next un-expanded state from the queue
        ...
        # print()
        if path < g[goaltest]:
            for action, s in state.successors():  # Go through all successors of state
                newCost = g[state] + action.cost
                if s not in g or newCost < g[s]:
                    g[s] = newCost
                    f = h(s)+g[s]
                    Q.put((f, s))
                    predecessor[s] = state
    ...
    # listActionOrdered=[]
    while (s!=start_state):
        for action, nextS in predecessor[s].successors():
            if nextS == s:
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------
    # Example 1
    grid_S = MAPPGridState(
        [(0, 0), (1, 1), (0, 1), (1, 0)], nrows=5, ncols=5, walls=[])
    grid_G = MAPPGridState(
        [(3, 3), (2, 2), (2, 3), (3, 2)], nrows=5, ncols=5, walls=[])
    # print(
    #     f"""
# ---------------------------------------------
# Example 1
